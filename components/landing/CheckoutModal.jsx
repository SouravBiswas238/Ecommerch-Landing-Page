...
const CheckoutModal = ({
  ...
  const [checkoutForm, setCheckoutForm] = useState({
    name: "", phone: "", address: "",
    deliveryType: "delivery", pickupTime: "", orderNote: "", paymentMethod: "cash",
  });
  ...
  useEffect(() => {
    setCheckoutForm((prev) => (!prev.pickupTime ? { ...prev, pickupTime: getNowPlus30Str() } : prev));
  }, []);

  // Auto-detect location for delivery
  useEffect(() => {
    if (checkoutForm.deliveryType === "delivery" && !mapLocation && navigator.geolocation) {
      setGeoLoading(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!checkoutForm.phone?.trim()) errors.phone = "Please enter your phone number";
    else if (!/^[0-9]{8,15}$/.test(checkoutForm.phone)) errors.phone = "Phone must be 8-15 digits only";
    if (checkoutForm.deliveryType === "delivery") {
      if (!checkoutForm.address?.trim()) errors.address = "Please enter your delivery address";
      if (!mapLocation) errors.mapLocation = "Please pin your location on the map";
    }
    ...
    setFormErrors({});
    await onSubmit(checkoutForm, mapLocation, cartTotal, cartSubtotal, deliveryFee);
  };
    ...
    <div className="fixed inset-0 z-50 bg-[#000]/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden animate-zoom-in border border-[#E5E7EB] flex flex-col max-h-[92vh]">
        ...
        <div className="text-white p-5 flex items-center justify-between shrink-0"
          ...
          <h3 className="font-extrabold text-base sm:text-lg">Delivery Details</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="space-y-1">
            ...
            />
            {formErrors.name && <p className="text-xs text-[#dc3545] font-semibold mt-1">{formErrors.name}</p>}
          </div>
          ...
          <div className="space-y-1">
            ...
            />
            {formErrors.phone && <p className="text-xs text-[#dc3545] font-semibold mt-1">{formErrors.phone}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[{ key: "delivery", label: "🚚 Delivery" }, { key: "pickup", label: "🏪 Pickup" }].map((opt) => (
                <button
                  key={opt.key} type="button"
                  onClick={() => { setCheckoutForm((p) => ({ ...p, deliveryType: opt.key, address: "" })); setFormErrors((p) => ({ ...p, address: "" })); }}
                  className={`py-3 rounded-xl border-2 text-xs font-bold transition-all cursor-pointer`}
                  style={checkoutForm.deliveryType === opt.key ? {
          ...
          {checkoutForm.deliveryType === "delivery" && (
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input
                  type="text" placeholder="Street name, apartment, building no, city"
                  className={`w-full bg-[#f9f9f9] border rounded-xl px-4 py-2.5 text-xs focus:outline-none transition-all`}
                  style={!formErrors.address ? { borderColor: "var(--color-border)" } : { borderColor: "var(--color-danger)" }}
                  value={checkoutForm.address}
                  onChange={(e) => { setCheckoutForm((p) => ({ ...p, address: e.target.value })); setFormErrors((p) => ({ ...p, address: "" })); }}
                />
                {formErrors.address && <p className="text-xs text-[#dc3545] font-semibold mt-1">{formErrors.address}</p>}
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-extrabold text-[#808080] uppercase tracking-wider">
                    🗺️ Pin on Map <span className="font-normal text-[#dc3545]">*</span>
                  </span>
                  {geoLoading ? (
                    <span className="text-[10px] font-bold animate-pulse" style={{ color: "var(--color-primary)" }}>📍 Detecting your location…</span>
                  ) : mapLocation ? (
                    <span className="text-[10px] text-[#198754] font-bold">✓ {mapLocation.lat.toFixed(5)}, {mapLocation.lng.toFixed(5)}</span>
                  ) : null}
                </div>
                {formErrors.mapLocation && <p className="text-xs text-[#dc3545] font-semibold mt-1">{formErrors.mapLocation}</p>}

                {/* Search */}
                <div className="flex gap-1.5">
                  <input
                    type="text" placeholder="Search place to pin..."
                  ...
                  />
                  <button
                    type="button" onClick={handleMapSearch} disabled={mapSearching}
                ...
                <div className="rounded-xl overflow-hidden border border-[#E5E7EB] h-44">
                  <MapPicker mapLocation={mapLocation} onSelect={handleMapSelect} />
                </div>
                <p className="text-[10px] text-[#808080]">Click anywhere on the map to drop a pin and auto-fill the address above.</p>
              </div>
            </div>
          )}
          ...
          {checkoutForm.deliveryType === "pickup" && (
            <div className="flex items-start gap-2 rounded-xl px-3 py-2.5 border"
              ...
              <p className="text-xs font-semibold" style={{ color: "var(--color-primary)" }}>
                You have selected <strong>Pickup</strong>. Please collect your order directly from our restaurant.
              </p>
            </div>
          )}
          ...
          {/* Order Note */}
          <div className="space-y-1">
            <label className="text-xs font-extrabold uppercase text-[#003660] block tracking-wide">Order Note (Optional)</label>
            <textarea
              ...
              onChange={(e) => setCheckoutForm((p) => ({ ...p, orderNote: e.target.value }))}
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                >
                  <span style={{ color: checkoutForm.paymentMethod === opt.key ? "var(--color-primary)" : "var(--color-muted)" }}>{opt.icon}</span>
          ...
          <div className="bg-[#f9f9f9] p-3.5 rounded-2xl border border-[#E5E7EB] text-xs space-y-1 text-[#808080]">
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span className="font-bold text-[#374151]">${cartSubtotal.toLocaleString()}</span>
            </div>
            {checkoutForm.deliveryType === "delivery" && (
              <div className="flex justify-between">
                <span>Delivery Fee</span>
                <span className="font-bold text-[#374151]">
                  {deliveryFee > 0 ? `$${deliveryFee.toLocaleString()}` : mapLocation ? "Calculating..." : "Select map location"}
                </span>
              </div>
            )}
            {deliveryError && (
          ...
          {!deliveryChargeLoading && !deliveryError && (
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.background = "var(--color-primary-hover)"; e.currentTarget.style.boxShadow = "0 8px 20px rgb(var(--color-primary-rgb) / 0.25)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "var(--color-primary)"; e.currentTarget.style.boxShadow = "0 4px 12px rgb(var(--color-primary-rgb) / 0.15)"; }}
            >
