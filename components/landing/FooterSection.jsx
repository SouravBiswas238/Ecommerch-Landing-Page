...
const FooterSection = ({ companyData }) => {
  ...
  const address =
    "Anglin Global Building, 30-34 Market St, Montego Bay, Jamaica";
  ...
  const whatsappUrl = "http://wa.me/+18768190186";

  const mapEmbedUrl = `https://maps.google.com/maps?q=${lat},${lng}&t=&z=16&ie=UTF8&iwloc=&output=embed`;

  return (
    <footer className="w-full mt-10 bg-white border-t border-[#3F9C9F] font-sans">
      <div className="mx-auto max-w-6xl px-4 pt-8 pb-4 md:px-6 lg:px-8">
          ...
          <div className="md:col-span-5 flex flex-col items-center md:items-start text-center md:text-left">
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 max-w-xs text-xs text-slate-400 leading-relaxed">
                  Authentic Jamaican flavors, warm service, and a vibrant
            ...
            <div className="flex items-center gap-2 mt-3 sm:pl-20 md:pl-0">
              <a
                ...
                className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#3F9C9F] text-white hover:bg-[#99C256] transition-colors"
              >
                <svg
              ...
              >
                <svg className="h-3.5 w-3.5 fill-current" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.966-.272-.099-.47-.148-.67.15-.198.297-.767.966-.94 1.164-.173.198-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.15-.174.198-.298.298-.496.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.67-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.298-1.04 1.016-1.04 2.479 0 1.462 1.065 2.876 1.212 3.074.149.198 2.095 3.2 5.077 4.487.71.306 1.262.489 1.693.625.711.226 1.359.194 1.871.118.571-.085 1.758-.719 2.006-1.413.247-.694.247-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982 1-3.651-.235-.374a9.861 9.861 0 0 1-1.51-5.256c.002-5.45 4.436-9.884 9.888-9.884 2.64 0 5.117 1.03 6.988 2.898a9.825 9.825 0 0 1 2.895 6.994c-.003 5.45-4.437 9.884-9.888 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.946L0 24l6.311-1.655a11.867 11.867 0 0 0 5.739 1.46h.005c6.552 0 11.886-5.335 11.889-11.893A11.821 11.821 0 0 0 20.464 3.488" />
                </svg>
              </a>
            </div>
          </div>
          ...
          <div className="md:col-span-7 flex flex-col gap-4 w-full">
            ...
            <div className="grid gap-4 sm:grid-cols-2 border-b border-slate-100 pb-4">
              ...
              <div className="flex gap-2.5">
                ...
                <div>
                  <h4 className="text-[10px] font-bold uppercase tracking-wider text-[#3F9C9F]">
                    Our Location
                  </h4>
              ...
              <a href={`tel:${phone}`} className="flex gap-2.5 group h-fit">
                <Phone
                  size={16}
                  className="text-[#3F9C9F] mt-0.5 shrink-0 group-hover:text-[#99C256] transition-colors"
                />
                <div className="flex flex-row items-baseline gap-2 flex-wrap">
                  <h4 className="text-[10px] font-bold uppercase tracking-wider text-[#3F9C9F] group-hover:text-[#99C256] transition-colors">
                    Hotline:
                  </h4>
                  <p className="text-sm font-bold text-slate-700 tracking-wide group-hover:text-[#3F9C9F] transition-colors">
                    {phone}
                  </p>
                </div>
              </a>
        ...
        <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-[10px] text-slate-400">
          <div>
            ...
            <span className="font-medium text-slate-600">{brandName}</span>. All
            rights reserved.
          </div>
