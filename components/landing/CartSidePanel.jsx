...
const CartSidePanel = ({
  ...
  <div className="bg-white rounded-2xl border p-5 shadow-sm sticky top-24 space-y-5" style={{ borderColor: "var(--color-border)" }}>
    {/* Header */}
    <div className="flex items-center justify-between pb-3 border-b" style={{ borderColor: "var(--color-border)" }}>
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-extrabold text-base" style={{ color: "var(--color-secondary)" }}>Your Order Checkout</h3>
      </div>
      <span className="px-2 py-0.5 rounded-full text-xs font-extrabold"
        style={{ background: "rgb(var(--color-primary-rgb) / 0.1)", color: "var(--color-primary)" }}>
    ...
    {cart.length === 0 ? (
      <div className="py-12 text-center space-y-3">
        <div className="w-12 h-12 rounded-full bg-[#f9f9f9] flex items-center justify-center mx-auto" style={{ color: "var(--color-muted)" }}>
          <ShoppingCart className="w-5 h-5" />
        </div>
        <div className="max-w-[180px] mx-auto">
          <p className="text-sm font-bold" style={{ color: "var(--color-secondary)" }}>Your cart is empty</p>
          <p className="text-xs mt-1" style={{ color: "var(--color-muted)" }}>Select items from our menu above to customize your feast.</p>
        </div>
      </div>
    ) : (
        ...
        {cart.map((item) => (
          <div key={item.customKey} className="flex items-start justify-between gap-3 text-sm pb-3 border-b" style={{ borderColor: "#f9f9f9" }}>
            <div className="flex-1 min-w-0">
              <h4 className="font-bold truncate" style={{ color: "var(--color-secondary)" }}>{item.product.name}</h4>
            ...
            <div className="flex flex-col items-end gap-1.5 shrink-0">
              <div className="flex items-center bg-[#f9f9f9] rounded-full p-0.5" style={{ border: "1px solid var(--color-border)" }}>
                ...
                <span className="w-5 text-center text-xs font-bold" style={{ color: "var(--color-secondary)" }}>{item.quantity}</span>
                <button
                  onClick={() => onUpdateQty(item.customKey, 1)}
                  className="w-5 h-5 rounded-full flex items-center justify-center hover:bg-white cursor-pointer"
                  style={{ color: "var(--color-primary)" }}
    ...
    {/* Totals & Actions */}
    {cart.length > 0 && (
      <div className="space-y-2 text-xs pt-2 border-t" style={{ borderColor: "var(--color-border)" }}>
        ...
        <div className="flex justify-between text-sm pt-2 border-t border-dashed" style={{ borderColor: "var(--color-border)" }}>
          <span className="font-extrabold" style={{ color: "var(--color-secondary)" }}>Total Order Amount</span>
        ...
        <div className="flex gap-2 pt-3">
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "rgb(220 53 69 / 0.05)"; e.currentTarget.style.borderColor = "var(--color-danger)"; }}
          ...
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "var(--color-primary-hover)"; e.currentTarget.style.boxShadow = "0 8px 20px rgb(var(--color-primary-rgb) / 0.2)"; }}
